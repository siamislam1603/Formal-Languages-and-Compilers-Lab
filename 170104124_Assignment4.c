...
void tokenizer(){
    ...
    if(strlen(lex)>0){
        ...
        if(found==0){
            ...
            if(strcmp(lex,"for")==0){
                ...
                if(colon!=2)
                    printf("Declaration of for loop not correct at line %d\n",ln-1);
                c=fgetc(p1);
                colon=0;
            }
            if(strcmp(lex,"(")==0)
    ...
    if(duplicateFound==0){
        ...
        fprintf(p3,"Misplaced \'%s\' at line %d,",parenthesis,ln-1);
        stop=1;
    }
    else if(duplicateFound==1 && ifElseBalanced==0 && stop==0){
        fprintf(p3,"Unmatched \'%s\' at line %d,",ifElse,ln-1);
        stop=1;
    }
...
void intermediateOutput(){
    p1 = fopen("s4_input.c", "r");
    p2 = fopen("assignment4_intermediateOutput.txt","w");
    p3 = fopen("assignment4_output.txt","w");
    if(!p1)
    ...
    p2 = fopen("assignment4_intermediateOutput.txt","r");
...
int main(){
    ...
    printf("\nOutput:\n");
    printf("--------\n");
    p2 = fopen("assignment4_output.txt","r");
    while((c=fgetc(p2))!=EOF)
		printf("%c",c);
    fclose(p2);
    printf("\n");
